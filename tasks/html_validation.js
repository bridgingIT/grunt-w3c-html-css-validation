    ...
    var validate = function () {
        ...
        var validate = function (files) {
            ...
            if (files.length) {
				...
				var fileChecksum = checksum(fs.readFileSync(files[counter], 'utf8'));

                if (grunt.file.exists(options.path)) {
                    readSettings = grunt.file.readJSON(options.path);
                }
                var currFileStat = readSettings[files[counter]] || { hash: fileChecksum, valid: undefined};
				
                if (currFileStat.hash === fileChecksum && currFileStat.valid === true) {
        ...
        if (options.remoteFiles) {
            ...
            for (var i = 0; i < dummyFile.length; i++) {
                files.push('_temp-validation.html');
            }
